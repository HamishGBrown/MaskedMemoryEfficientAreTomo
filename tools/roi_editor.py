...
Usage
-----
    python3 roi_editor.py stack.mrc [--ang angles.txt] [--roi existing.txt] [-o out.txt]
...
Tile-grid overlay
-----------------
    Cyan dashed rectangles show the tile boundaries AreTomo2 would use
...
class RoiEditor:
    ...
    def _draw_tiles(self) -> None:
        ...
        for iy in range(self.grid_ny):
            for ix in range(self.grid_nx):
                r = Rectangle(
                    (ix * pw, iy * ph), pw, ph,
                    linewidth=0.8, edgecolor='cyan', facecolor='cyan',
        ...
        if self._tile_rects:
            ...
            t = self.ax.text(
                ...
                color='cyan', fontsize=6, alpha=0.8, zorder=8,
    ...
    def _redraw_labels(self) -> None:
        ...
        for i, (x, y) in enumerate(self.points):
            t = self.ax.text(x + self.img_w * 0.005, y + self.img_h * 0.005,
                             str(i + 1), color='yellow', fontsize=7,
    ...
    def _cb_save(self, _):
        save_roi(self.points, self.out_path)
