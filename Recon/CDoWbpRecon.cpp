...
static int   s_iOutFileFd  = -1;      // file descriptor kept open during reconstruction
static size_t s_tDataOffset = 0;       // byte offset of first slice in the file
...
{	s_pTomoStack = pTomoStack;
	...
	// written to disk. Y rows outside the ROI are skipped entirely (each
	// row is an independent 2D reconstruction), while X columns outside
	// the ROI are simply not copied into the output buffer in
	// mGetReconResult() — the reconstruction kernels never see the crop.
	//--------------------------
	...
	//--------------------------
	// Write MRC header and obtain a raw file descriptor for pwrite().
	// CSaveMrc opens, truncates and writes a 1024-byte main header plus an
	// ext-header stub (32 floats × numY bytes = 128*numY bytes) with zeroed
	// placeholder stats.  We then close it and reopen O_RDWR so that the
	// per-thread pwrite() calls can land at the correct offsets.
	//--------------------------
	{	Mrc::CSaveMrc aSaveMrc;
		...
		aSaveMrc.SaveMinMaxMean(0.0f, 0.0f, 0.0f); // placeholder; updated below
		aSaveMrc.m_pSaveMain->DoIt();
		// CloseFile() rewrites the header and closes the fd.
	}
	//--------------------------
	// ext-header size = 32 floats × sizeof(float) × numY = 128 × numY bytes.
	// Data follows immediately after main (1024 B) + ext header.
	//--------------------------
	...
	// Pre-allocate the full file so pwrite() to any slice offset is safe.
	ftruncate(s_iOutFileFd, (off_t)tTotalBytes);
	//--------------------------
	// Per-slice stats arrays (layout: numY × [min, max, mean]).
	...
	delete[] pThreads;
	printf("WBP reconstruction completed.\n\n");
	//--------------------------
	...
	s_iOutFileFd = -1;
	//--------------------------
	// Return a metadata-only stub; callers use IsStreaming() to detect this.
...
void CDoWbpRecon::ThreadMain(void)
{
	...
	//---------------------------------
	m_aTomoWbp.Setup(s_iFullVolX, s_aiVolSize[1],
...
//-----------------------------------------------------------------------------
// Runs after cudaStreamSynchronize, so m_pfVolXZ is fully populated.
...
//    pwrite() is thread-safe for non-overlapping file regions, so no mutex
//    is needed when each GPU thread writes a distinct slice index.
//-----------------------------------------------------------------------------
void CDoWbpRecon::mGetReconResult(int iLastY)
{
	...
	//--------------------------------
	// In-place z-flip: swap row z with row (iSliceZ-1-z).
	...
	//--------------------------------
	// Per-slice stats (written to distinct indices — no race condition).
	float fMin = 1e30f, fMax = -1e30f;
	double dSum = 0.0;
